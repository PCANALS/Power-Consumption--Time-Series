...
write.csv(power_new2, file = "All_variables.csv")
...
write.csv(power_ym, file = "power_ym.csv")
...
write.csv(power_df_d, file = "power_df_d.csv")
...
power_d_tslm_for_2<-forecast(power_d_tslm_2, h = 730)

Mtslm2<-accuracy(power_train_d_tslm_for_2,power_test_d_ts_2)

#plot prediction by day2#
p_tslm_d2<-autoplot(power_d_ts) + 
  autolayer(power_d_tslm_for_2, PI=FALSE, col="lightseagreen", size=1)+
  ggtitle("By Day") +
  xlab("Time") +
  ylab("kWm")
...
p_tslm_w2<-autoplot(power_w_ts) + 
  autolayer(power_w_tslm_for_2, PI=FALSE, col="lightseagreen", size=1)+
  ggtitle("By Week") +
  xlab("Time") +
  ylab("kWm")
...
p_all_for<-grid.arrange(p_tslm_m2, p_tslm_w2, p_tslm_d2, top = "Forecasting Active Power Consumption")
...
#   autolayer(power_d_HW_for, PI=FALSE, col="lightseagreen", size=1)
# 
# 
...
ui<- dashboardPage(dashboardHeader(title = "Hola"), 
                   dashboardSidebar(
                     sidebarMenu(
                       menuItem("Dashh", tabName = "dash", icon=icon("ambulance")),
                       menuItem("Wid", tabName = "wid", icon = icon("fish")))), 
                   dashboardBody(
                     tabItems(
                       ...
                       tabItem(tabName = "wid",
                               fluidRow(
                                 box(plotOutput("plot3"), heigth = 250),
                                 box(title = "Petal Slider", 
...
server<-function(input, output){
  ndf<-reactive({iris%>%filter(Petal.Length>=input$slid[1] & Petal.Length<= input$slid[2])})
  output$plot2<-renderPlot({
    p_tslm_m2
  })
  output$plot3<-renderPlot({new<-ndf()
  hist(new$Petal.Length)
  })
